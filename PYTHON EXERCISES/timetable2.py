...
class ExamScheduler:
    ...
    def print_timetable(self, timetable):
        ...
        print("=" * 90)
        print(f"{'Date':<12} {'Morning Session (9:00 AM - 12:00 PM)':<38} {'Afternoon Session (2:00 PM - 5:00 PM)':<38}")
        print("-" * 90)
        
        for date in sorted(timetable.keys()):
            print(f"{date:<12} "
                  f"{timetable[date]['morning']:<38} "
                  f"{timetable[date]['afternoon']:<38}")
        
        print("=" * 90)
        print(f"\nTotal examination days: {len(timetable)}")

# Run the program and display the timetable
